...
def get_most_recent_update_time(directory: Path) -> float:
    ...
    try:
        ...
        logging.error(f"Error getting modification times: {e}")
        return 0
...
def is_zip_complete(zip_path: Union[str, Path], testzip=False) -> bool:
    try:
        ...
        logging.error(f"Zip file {zip_path} is invalid: {e}")
        return False
...
def is_parquet_complete(path: Path) -> bool:
    try:
        ...
        logging.error(f"Parquet file {path} is incomplete or corrupted: {e}")
        return False
...
async def wait_for_downloads_to_complete(
    ...
    logging.error(f"Timeout waiting for {files} after {timeout_seconds} seconds")
    return False
