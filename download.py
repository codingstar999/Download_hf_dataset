...
def list_hf_files(repo_id, repo_type="dataset", extension=None):
    ...
    try:
        ...
        logging.error(f"Failed to list files of type {extension} in {repo_id}: {e}")
    return files
...
def openvid1m_err_handler(
    ...
    for part_url in part_urls:
        ...
        if part_file_path.exists():
            logging.warning(f"File {part_file_path} exists.")
        ...
        try:
            ...
            logging.info(f"File {part_url} saved to {part_file_path}")
            downloaded_parts.append(part_file_path)
    ...
    if len(downloaded_parts) == len(part_urls):
        try:
            ...
            logging.info(f"Successfully combined parts into {combined_file}")
